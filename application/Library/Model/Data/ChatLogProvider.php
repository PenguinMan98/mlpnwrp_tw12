...
class Model_Data_ChatLogProvider extends Model_Data_ChatLogProviderBase
{
	...
	// toggle private or not
	// search by player (unused)
	public function adminSearch( $userId, $startDate, $endDate = null, $text = null, Model_Structure_ChatRoom $room = null, $characters = null, $private = false){//Model_Structure_Phpbb_Users $player = null
		/*if(empty($startDate)){
			throw new Exception("You must specify a start date.");
		}*/
		if(empty($text) && empty($characters) && empty($player) && empty($startDate)){
		...
		if(!is_null($characters) && $characters != "" && !$private){
			...
			$charWhere = array();
			if(count($charArr) == 1){
				$charWhere[] = " ( `handle` = '{$charArr[0]}' OR `recipient_username` = '{$charArr[0]}' ) ";
			}else{
				foreach($charArr as $char1){
					if(empty($char1)) continue;
					foreach($charArr as $char2){
						if(empty($char2)) continue;
						if($char1 == $char2) continue;
						// Two possibilities.
						$whereStr = " ( ";
						
						// 'handle' and 'recipient_username'
						$whereStr .= " ( `handle` = '{$char1}' AND `recipient_username` = '{$char2}' ) ";
						$whereStr .= " OR ( `handle` = '{$char2}' AND `recipient_username` = '{$char1}' ) ";
...
WHERE `handle`= ? AND `chat_rand` = ?';
	...
	/*
	 * Probably the most important query in the site.
	 * */
...
UPDATE `chat_log` set `$col`= ? WHERE `chat_log_id` = ?
	...
	public function getAllPostsAfterId( $chatLogId ){
		$strSql = "
SELECT * FROM `chat_log` WHERE `chat_log_id` > ? ORDER BY `chat_log_id` ASC limit 10000
		";
		$arrParams = array( $chatLogId );
		return $this->getArrayFromQuery($strSql, $arrParams);
	}
	
	public function wwuPostCount(  ){
		$strSql = "
select 
	count(*) as 'ic_posts'
from chat_log cl
where cl.timestamp >= 1425694356
and cl.chat_log_type_id = 1
and chat_room_id > 1 AND chat_room_id < 8
		";
		$arrParams = array(  );
		$arrErrors = array();
		$arrResults = array();
		DAO::getAssoc($strSql, $arrParams, $arrResults, $arrErrors);
		return $arrResults[0]['ic_posts'];
	}
}
