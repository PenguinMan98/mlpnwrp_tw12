...
if(isset($roomId)){
	$searchRoom = $chatRoomProvider->getOneByPk($roomId);
}
...
try{
	$results = $chatLogProvider->search( $userId, $startDate, $endDate, $text, $searchRoom, $searchCharacterList, ($showPrivate && $loggedIn) );
}catch(Exception $e){
	$results = array();
