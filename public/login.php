...
.form_error{
	color: red;
	display: <?php echo (!empty($systemMessage))? 'block':'none' ?>;
}
		...
		<div id="login">
			...
			<div id="tabs-1">
				<form id="login_form" action="chat/" method="POST">
					...
					<input type="hidden" id="loggedIn" name="loggedIn" value="<?php echo ($loggedIn)?1:0?>">
...
$(function() {
	$('#login').tabs();
	$('#forum_login').on('click', function( event ){
		if(!loggedIn){
			event.preventDefault();
		}
		logMeIn();
	});
	$('#guest_login').on('click', function( event ){
		event.preventDefault();
		logGuestIn();
	});
...
function logGuestIn(){
	...
	$.ajax({
		url: "<?php echo SITE_ROOT?>/chat/ajax-chat/php/login_guest.php",
