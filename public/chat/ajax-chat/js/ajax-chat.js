...
var currentWeatherStateId = 1;// clear
...
function showExitPM(){
	...
	$('#exit_pm').css('opacity', 1);
	$('#exit_pm').css('right', '220px');
...
function hideExitPM(){
	$('#exit_pm').css('opacity', 0);
	$('#exit_pm').css('right', '0px');
	$('#exit_pm').hide();
...
function showRooms(){
	...
	$('#rooms').show();
	// animate moving right to 0
	console.log('show rooms animation', $('#rooms'), $('#rooms').animate );
	$('#rooms').css('opacity', 1);
	$('#rooms').css('right', '220px');

	/*$('#rooms').animate({
			opacity: '1',
			right: '220px'
		},1000,'swing',function(){
			console.log('show rooms animation complete');
		});*/
}
function hideRooms(){
	// animate moving left to 0
...
function chat_msgs_add()
{
    ...
    if(post_text.indexOf('/msg') == 0){
    	...
    	var varTo = varSplit[1];
    	var varMessage = varSplit.slice(2).join(' ');
    	console.log( 'sending to ', varTo, ' message ', varMessage);
    	sendCharacterMessage( varTo, varMessage );
    	$('#send').val('');
    	//variables[varName] = varValue;
    	//console.log('variables: ', variables);

    	chat_out_system_msg( 'message sent to ' + varTo );

    	return true;
    }
	// weather
    if(post_text.indexOf('/setWeather') == 0){
    	var varSplit = post_text.split(' ');
    	if(varSplit.length < 3){ // token, state, duration
    		console.log('not enough arguments');
    		// error: not enough arguments.
    		return false;
    	}
    	var weatherToken = varSplit[0];
    	var weatherVarState = varSplit[1];
    	var weatherVarDuration = varSplit[2];
    	var weatherTheRest = varSplit.slice(3).join(' ');
    	// add the chat room id into the string automagically
    	console.log( 'post text before', post_text);
    	//post_text = weatherToken + ' ' + weatherVarState + ' ' + weatherVarDuration + ' ' + chat_room + ' ' + weatherTheRest;
    	console.log( 'post text after', post_text);
    	$('#send').val(''); // clear out the message window
    	
   		sendWeatherChange( weatherToken, weatherVarState, weatherVarDuration, chat_room );
    	return true;
    }
	...
	.done(function(response) {
		if(response.success){
	      ...
	      if(typeof(response.newWeatherState) != "undefined" && typeof(response.newWeatherState.weather_state_id) != "undefined"){
	    	  ...
	    	  chatState.weatherStateId = parseInt(response.newWeatherState.weather_state_id);
	    	  weatherState = response.newWeatherState;
	      }
	      updateWeather( response.serverTime );
	      //-----------ADMIN MOVE---------------
	      if(typeof(response.toAdminRoom) != "undefined"){
	    	  chatState.roomId = response.toAdminRoom;
			  chat_api_onload(chatState.roomId, character_id != 'G', handle);
	      }
	      
	      //-----------WWU---------------
	      if(typeof(response.wwu_count) != "undefined"){
	    	  if( chatState.roomId > 0 && chatState.roomId < 8 ){
	    		  var before = 1200;
	    	  	  var after = response.wwu_count - 1200;
	    	  	  var totalCount = before + 2 * after;
	    		  $('#wwu').html( 'Winter Wrapped Up: ' + Math.min( parseInt(totalCount * 10000 / 2250 ) / 100, 100 ) + '%' );
	    	  }else
	    		  $('#wwu').html( '' );
	      }else{
	    	  $('#wwu').html( '' );
...
function chat_msgs_usr(handle, color, sidebar)
{
  ...
  if(sidebar && thisChar && !thisChar.cutie_mark && thisChar.registered){
	  ...
	  retString += '<span style="cursor: pointer;" onClick="showHUD(this, \''+handle+'\'); return false;">G&nbsp;</span>';
...
function chat_out_msgs()
{
    ...
    if($('#autofocus').attr('checked')){ //Disable autofocus!
    	// Why do it this way?  Because the original coder empties the chat window and replaces
    	// all the posts with new ones and then you have to scroll to the bottom.
    	// this is complicated by not knowing when images or whatever will load.
    	// meaning that the bottom that it finds when it first runs might not 
    	// be the bottom a half a second later.  So, check immediately and every tenth of a second
    	// afterward for a half a second.  It works out pretty seamless but it's just a bandaid.
    	// The real solution is to stop emptying and reloading the chat pane.
...
function showHUD(element, charName){
	...
	//console.log('showHUD called', element, charName);
	$.ajax({
	...
	.done(function(response) {
		...
		if(response.success && $('#character_info_base').css('display') == 'none'){
			...
			if( response.type == 'registered' ){
				...
				$('#hud_character_name').html('<a title="Open Character Profile" href="'+SITE_ROOT+'/chat/character/view.php?c='+response.characterInfo.name+'" target="_blank">' + response.characterInfo.name + '</a>');
				
				// set the player name. Link to player profile
				//$('#hud_player_name').html(response.characterInfo.username);
				$('#hud_player_name').html('<a title="Open Player Character List" href="'+SITE_ROOT+'/chat/player/characters.php?p='+response.characterInfo.username+'" target="_blank">' + response.characterInfo.username + '</a>');
			}else if( response.type == 'registered-guest' ){
				// set the character name.  Link to character profile
				$('#hud_character_name').html( response.characterInfo.handle );
				...
				// set the character name.  Link to character profile
				$('#hud_character_name').html( 'Guest: ' + response.characterInfo.handle );
				
				// set the player name. Link to player profile
				//$('#hud_player_name').html(response.characterInfo.username);
				$('#hud_player_name').html( 'Guest<!--' + response.characterInfo.guest_ip + '-->' );
			}

				// This must be fixed for the away/Active feature to work properly
			var ts = response.server_time;
			var fifteenMinutes = 60*15;
			var fiveMinutes = 60*5;

			if(	ts - response.characterInfo.last_status_request > fiveMinutes ){
				$('#hud_activity_status').html('Player Disconnected');
			}else if(	ts - response.characterInfo.last_activity > fifteenMinutes ){
				$('#hud_activity_status').html('Player is Away');
			}else{
				$('#hud_activity_status').html('Player is Active');
			};

			// set the room.  Link to change rooms.
			$('#hud_room').html( 'In: ' + roomList[response.characterInfo.chat_room_id]);
			$('#hud_room').off('click');
			...
			if(stare_array.indexOf(response.characterInfo.name) != -1){
				...
				$("#stare").attr("title","Stare at this pony (highlight)");
			}
			...
			$('#character_info_base').css( 'top', linkRect.bottom - ( modalRect.bottom - modalRect.top  ) );
		}else{
			// hide it
...
function sendCharacterMessage( characterString, message ){
	$.ajax({
		url: chat_path+"php/send_character_message.php",
		method: 'POST',
		data: {
			character_id: character_id,
...
function updateWeather( serverTime ){
	...
	if(d.getHours() < 10 || d.getHours() >= 22 ){ // nighttime
		timeOfDay = 'night';
	...
	if( season != currentSeason || timeOfDay != currentTimeOfDay || weatherState.weather_state_id != currentWeatherStateId ){ // if I need to change
		if( icon ){
		...
		currentWeatherStateId = weatherState.weather_state_id;
	}
}

function sendWeatherChange( weatherToken, weatherVarState, weatherVarDuration, chat_room ){
	var method = ( weatherToken == '/setWeather' ) ? 'room': 'group';
	$.ajax({
		url: chat_path+"php/set_weather.php",
		method: 'POST',
		data: {
			method: method,
			state: weatherVarState,
			duration: weatherVarDuration,
