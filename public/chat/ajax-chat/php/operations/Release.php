 ...
 * */
class Operation_Release extends OperationBase{
	public $operator;
	public $data;
	...
	private function execute(){
		...
		try{
			$character->setChatRoomId( 1 );
			$cProvider->updateOne( $character, $arrErrors );
			
			if(!empty($arrErrors)){
				$this->messages[] = "Something went wrong trying to release $characterName.";
		...
		return "{{ $characterName has been released }}";
	}
}