...
$character = $characterHelper->getOneByCharacterNameAndId($handle, $userId);
...
if(is_numeric($room) && $room > 0){
	if(is_object($character) && $character->getChatRoomId() != $room )
	{
		$response->success = false;
		$response->error = "You aren't in that room.";
		die(json_encode($response));
	}
	if(is_object($guestUser) && $guestUser->getChatRoomId() != $room )
	{
		$response->success = false;
		$response->error = "You aren't in that room.";
		die(json_encode($response));
...
print_r($recentPosts);
echo "</pre>";
echo "Flood Check: " . (count($recentPosts)) . ">=" . ($FLOODCUTOFFPOSTS) . "|" . (count($recentPosts) >= $FLOODCUTOFFPOSTS) . "<br>";
    	if(count($recentPosts) >= $FLOODCUTOFFPOSTS){
	    	...
	    	if($floodCheckPost){
	    		...
	    		echo "timediff($timeDiff) = time(".time().") - floodcheck({$floodCheckPost['timestamp']})";
	    		$flood = $timeDiff < $FLOODCUTOFFTIME;
	    		echo "flood($flood) = timediff($timeDiff) < floodcutoff($FLOODCUTOFFTIME)<br>";
	    	}
    	}
